...
impl InnerWebView {
  ...
  fn new_ns_view(
    ...
    // SAFETY: new_ns_view is only called on the main thread. On macOS 26 (Tahoe),
    // MainThreadMarker::new() incorrectly returns None during early app startup
    // inside applicationDidFinishLaunching: — the OS changed when the main thread
    // marker is considered valid. We bypass the check here since the call site is
    // provably on the main thread. Upstream: https://github.com/tauri-apps/tao/issues/1171
